    ...
    .then((data) => {
      ...
      const tasks = slideshowNodes.map((node) => {
        ...
        return loadImages(slug).then((images) => {
          ...
          window.SlideshowManager.createSlideshow({
            container: node,
            images,
      ...
      Promise.all(tasks).then(() => {
        if (document.visibilityState === 'visible') {
          window.SlideshowManager.setGlobalPaused(false);
        } else {
          window.SlideshowManager.setGlobalPaused(true);
        }
      });
    })
    .catch(() => {
